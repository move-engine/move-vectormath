...
inline void test_quat()
{
    ...
    // Rotation testing
    {
        ...
        THEN("Look rotation aligns the forward direction")
        {
            const quat look_forward =
                quat::look_rotation(vec3::forward(), vec3::up());
            const quat look_right =
                quat::look_rotation(vec3::right(), vec3::up());
            REQUIRE(move::math::same_rotation(look_forward, quat::identity(),
                                              component_type(0.001)));
            REQUIRE(move::math::approx_equal(vec3::forward() * look_right,
                                             vec3::right(),
                                             component_type(0.001)));
            REQUIRE(quat::look_rotation(vec3::zero(), vec3::up()) ==
                    quat::identity());
        }
        ...
        THEN("Logarithm and exponential are inverse operations")
        {
            ...
            REQUIRE(move::math::approx_equal(rotation, exp_result,
                                             component_type(0.001)));

            // Also test static versions
            quat static_log = quat::ln(rotation);
            quat static_exp = quat::exp(static_log);
            REQUIRE(move::math::approx_equal(rotation, static_exp,
                                             component_type(0.001)));
            REQUIRE(move::math::approx_equal(log_result, static_log,
                                             component_type(0.001)));
        }

        THEN("Logarithm handles negative-w and non-unit quaternions")
        {
            const quat negative_w = quat::angle_axis(
                vec3::x_axis(), move::math::deg2rad(component_type(270)));
            REQUIRE(move::math::approx_equal(negative_w.ln().exp(), negative_w,
                                             component_type(0.001)));

            const quat non_unit(component_type(1), component_type(2),
                                component_type(3), component_type(4));
            REQUIRE(move::math::approx_equal(non_unit.ln().exp(), non_unit,
                                             component_type(0.001)));
        }
        ...
        THEN("Identity quaternion logarithm is zero")
        {
            ...
            REQUIRE(move::math::approx_equal(log_identity, zero_quat,
                                             component_type(0.001)));
        }
        ...
        THEN("Zero quaternion exponential is identity")
        {
            ...
            REQUIRE(move::math::approx_equal(exp_zero, identity,
                                             component_type(0.001)));
        }

        THEN("Logarithm of quaternion multiplication is addition of logarithms")
        {
            // This is a property of quaternion logarithms for commuting
            // quaternions For rotations around the same axis: ln(q1 * q2) ≈
            // ln(q1) + ln(q2)
            quat rotation1 = quat::angle_axis(
                vec3(1, 0, 0), move::math::deg2rad(component_type(30)));
            quat rotation2 = quat::angle_axis(
                vec3(1, 0, 0), move::math::deg2rad(component_type(60)));
            ...
            REQUIRE(move::math::abs(log_combined_x - log_sum_x) <
                    component_type(0.001));
            REQUIRE(move::math::abs(log_combined_y - log_sum_y) <
                    component_type(0.001));
            REQUIRE(move::math::abs(log_combined_z - log_sum_z) <
                    component_type(0.001));
            REQUIRE(move::math::abs(log_combined_w - log_sum_w) <
                    component_type(0.001));
        }
    }
}
