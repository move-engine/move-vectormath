...
#define MOVE_VECTORMATH_HPP_INCLUDED 1
#endif
...
namespace DirectX
{
    namespace SimpleMath
    {
        ...
        struct Rectangle
        {
            ...
            constexpr Rectangle(
                int32_t ix, int32_t iy, int32_t iw, int32_t ih) noexcept
        ...
        struct Matrix4 : public XMFLOAT4X4
        {
            ...
            explicit Matrix4(_In_reads_(16) const float* pArray) noexcept
                : XMFLOAT4X4(pArray)
            {
            }
            Matrix4(CXMMATRIX M) noexcept
            {
            ...
            // Static functions
            static Matrix4 CreateBillboard(const Vector3& object,
                const Vector3& cameraPosition, const Vector3& cameraUp,
                _In_opt_ const Vector3* cameraForward = nullptr) noexcept;

            static Matrix4 CreateConstrainedBillboard(const Vector3& object,
            ...
            static Matrix4 create_transformation_complex(
            ...
            static Matrix4 create_model_matrix(const Vector3& scaling,
                const Quaternion& rotation,
                const Vector3& translation) noexcept;

            static Matrix4 CreateTranslation(const Vector3& position) noexcept;
            static Matrix4 CreateTranslation(
            ...
            static Matrix4 CreateFromAxisAngle(
            ...
            static Matrix4 CreatePerspective(float width, float height,
                float nearPlane, float farPlane) noexcept;
            static Matrix4 CreatePerspectiveOffCenter(float left, float right,
            ...
            static Matrix4 CreateLookAt(const Vector3& position,
                const Vector3& target, const Vector3& up) noexcept;
            static Matrix4 CreateWorld(const Vector3& position,
            ...
            // (roll)
            static Matrix4 CreateFromYawPitchRoll(
        ...
        struct Plane : public XMFLOAT4
        {
            ...
            static Plane Transform(
                const Plane& plane, const Matrix4& M) noexcept;
        ...
        public:
            ...
            Vector3 Project(const Vector3& p, const Matrix4& proj,
                const Matrix4& view, const Matrix4& world) const noexcept;
            void Project(const Vector3& p, const Matrix4& proj,
                const Matrix4& view, const Matrix4& world,
                Vector3& result) const noexcept;

            Vector3 Unproject(const Vector3& p, const Matrix4& proj,
                const Matrix4& view, const Matrix4& world) const noexcept;
            void Unproject(const Vector3& p, const Matrix4& proj,
                const Matrix4& view, const Matrix4& world,
                Vector3& result) const noexcept;
...
namespace std
{
    ...
    template <>
    struct less<DirectX::SimpleMath::Matrix4>
    {
        bool operator()(const DirectX::SimpleMath::Matrix4& M1,
            const DirectX::SimpleMath::Matrix4& M2) const noexcept
        {
...
namespace move
{
    ...
    using matrix4 = DirectX::SimpleMath::Matrix4;
    ...
    namespace LowLevelMath = DirectX;
}  // namespace move